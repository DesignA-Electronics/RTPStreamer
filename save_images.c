...
static int decode_rtp_packet(uint8_t *packet, int len, SDL_Renderer *renderer, SDL_Texture **texture)
{
	...
	if (offset == 0) {
		...
		if (fp)
			fclose(fp);
		sprintf(name, "foo-%5.5d.jpg", index);
		printf("Opening %s\n", name);
		index++;
		fp = fopen(name, "wb");
		write_jpeg_header(fp, 800, 600);
	}
	if (fp) {
		fwrite(payload, payload_len, 1, fp);
	}
#endif
	//printf("offset: %d len: %d, total: %d vs %d\n", offset, payload_len, offset + payload_len, sizeof(buffer));
	...
	if (marker) {
		if (pos > 0) {
			...
			int len = MakeHeaders(tmp, type, width, height, lqt_cqt, lqt_cqt + 64, 128, 0);
...
int main() {
	...
	if (nframes % 50 == 0) {
		...
		printf("Received %d frames in %ds (%f fps)\n",
				nframes, elapsed, elapsed ? (float)nframes / (float)elapsed : -1);
	}
